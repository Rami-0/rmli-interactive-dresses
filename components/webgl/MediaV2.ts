...
interface MediaV2Props {
  ...
  onClick?: (media: MediaV2) => void;
  reverseX?: boolean; // Reverse X positioning for right-to-left
  scaleMultiplier?: number; // Custom scale multiplier for individual images
}

export default class MediaV2 {
  extra: number;
  geometry: any;
  gl: WebGLRenderingContext | WebGL2RenderingContext;
  image: string;
  ...
  constructor({ geometry, gl, image, index, length, renderer, scene, screen, text, viewport, onClick, reverseX = false, scaleMultiplier = 1.0 }: MediaV2Props) {
    this.extra = 0; // Not used for non-infinite scroll, but kept for compatibility
  ...
  createShader() {
    ...
    // Set crossOrigin to prevent CORS issues on mobile
    image.crossOrigin = 'anonymous';

    image.src = this.image;
    image.onload = () => {
      // Set texture image - OGL will handle the WebGL texture upload
      texture.image = image;
      this.program.uniforms.uImageSizes.value = [image.naturalWidth, image.naturalHeight];
      
      // Force texture initialization on mobile to prevent context loss
      // Access the texture's internal WebGL texture to ensure it's created
      if (this.renderer && this.renderer.gl) {
        // Trigger a render to ensure texture is uploaded to GPU
        // This is especially important on mobile devices
        requestAnimationFrame(() => {
          // Texture should now be initialized in GPU memory
        });
      }
    };
    ...
    image.onerror = () => {
      console.error('[MediaV2] Failed to load image:', this.image);
    };
  }
  ...
  update(scroll: { current: number; last: number; target: number }, direction: 'left' | 'right') {
    // SIMPLIFIED: No infinite scroll wrapping, just simple positioning
    // 1. Horizontal positioning - controls spacing along X axis
    if (this.reverseX) {
      this.plane.position.x = scroll.current - this.x;
    } else {
    ...
    // 3.5. SCALE EFFECT - make center items MUCH bigger, side items smaller
    //    Creates dramatic focus on the active/center item
    //    Enhanced scale curve for more pronounced effect
    
    //    Normalize X position to 0-1 range (distance from center)
    //    Using a tighter normalization for sharper focus
    const normalizedXForScale = Math.abs(this.plane.position.x) / (this.viewport.width * 0.8);
    const clampedDistance = Math.min(normalizedXForScale, 1.0);
    
    //    SCALE CURVE - center item is 1.8x, side items scale down to 0.5x
    //    Using smooth easing curve (ease-out cubic) for natural transition
    const maxScale = 1.8;  // Center item scale (much bigger)
    const minScale = 0.5;  // Edge item scale (much smaller)
    
    //    Smooth easing curve: 1 - (1 - t)^3 for ease-out cubic
    const easedDistance = 1 - Math.pow(1 - clampedDistance, 3);
    let scaleFactor = maxScale - (maxScale - minScale) * easedDistance;
    
    //    Apply custom scale multiplier - affects the final size proportionally
    //    This adjusts the size when active (centered) while maintaining the scale curve
    //    The multiplier is applied to the scaleFactor, so it affects all positions proportionally
    scaleFactor *= this.scaleMultiplier;
    ...
    // 4. OPACITY FADE - Center item is fully visible, others fade dramatically
    //    Enhanced fade for stronger focus on active element
    const distanceFromCenter = Math.abs(this.plane.position.x);

    //    TIGHTER FOCUS AREA - sharper focus zone for more dramatic effect
    const focusWidth = this.viewport.width * 0.15;

    //    Calculate opacity: 1.0 at center, fading to minimum opacity at edges
    //    Using sharper falloff for more dramatic effect
    const normalizedDistance = Math.min(distanceFromCenter / focusWidth, 1);

    //    LOWER MINIMUM OPACITY - side items are more faded for stronger contrast
    const minOpacity = 1;

    //    Sharper fade using cubic ease-out for more dramatic transition
    const easedOpacityDistance = 1 - Math.pow(1 - normalizedDistance, 3);
    const opacity = 1.0 - (1.0 - minOpacity) * easedOpacityDistance;
    ...
    // NO INFINITE SCROLL - removed wrapping logic for 3-element carousel
  }
  ...
  onResize({ screen, viewport }: { screen?: { width: number; height: number }; viewport?: { width: number; height: number } } = {}) {
    ...
    // Use consistent scale calculation for both mobile and desktop
    // This ensures items are properly sized and visible

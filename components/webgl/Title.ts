...
// Fonts will be loaded at runtime from public directory
const fontPath = '/fonts/freight.json';
const src = '/fonts/freight.png';
...
export default class Title {
  ...
  constructor({ gl, plane, renderer, text }: { gl: WebGLRenderingContext | WebGL2RenderingContext; plane: any; renderer: any; text: string }) {
    AutoBind(this);
    ...
    this.createShader();
    this.createMesh(); // Will be async but called without await
  }
  ...
  async createMesh() {
    try {
      // Load font JSON at runtime using font loader
      ...
      console.error('Failed to create title mesh:', error);
    }
  }
}

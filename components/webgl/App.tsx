...
const Image9 = '/images/9.jpg';
const Image10 = '/images/10.jpg';
const Image11 = '/images/11.jpg';
const Image12 = '/images/12.jpg';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const initApp = async () => {
      ...
      // Clear existing medias and background from scene
      if (app.scene) {
        if (app.medias) {
    ...
    if (containerRef.current) {
      ...
      // Ensure canvas is not already in the DOM
      ...
      // Ensure canvas is visible (reset opacity in case it was hidden)
    ...
    // Preload fonts before creating media items
    await preloadFonts();

    const medias = mediasImages.map(({ image, text }, index) => {
      const media = new Media({
        ...
        onClick: (mediaItem) => {
          ...
          const itemIndex = mediaItem.index % (mediasImages.length / 2);
          router.push(`/detail/${itemIndex}`);
        },
      });
    ...
    mediasImages.forEach(({ image: source }) => {
      ...
      image.onload = () => {
        ...
        if (loaded === mediasImages.length) {
          ...
          // Ensure canvas becomes visible after loading
    ...
    const onTouchUp = (event: MouseEvent | TouchEvent) => {
      ...
      // Check for click/tap on media items

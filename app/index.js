...
export default class App {
  ...
  onTouchUp (event) {
    ...
    if (this.start !== undefined && this.startY !== undefined) {
      ...
      const deltaX = Math.abs(this.start - endX)
  ...
  /**
   * Handle click on canvas to detect which media item was clicked.
   */
  handleClick (clientX, clientY) {
    if (!this.medias || !this.gl.canvas) return
    ...
    for (let i = this.medias.length - 1; i >= 0; i--) {
      ...
      if (media.checkIntersection(x, y)) {
        ...
        break // Only trigger the topmost item
      }
    }
  }
